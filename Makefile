...
image-build:  ## Builds Docker Image
	docker build -t nesssna-demo:latest .

.PHONY: image-serve
image-serve:  image-build ## Runs Docker Image
	docker run -it --rm --init -p 8000:8000 nesssna-demo:latest

.PHONY: image-exec
image-exec:  image-build ## Runs Docker Exec
	docker run -it nesssna-demo:latest /bin/sh
